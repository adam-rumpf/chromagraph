...
for (var i = -4; i <= seg+4; i++)
{
	var pt = scr_rotate_vector(lv, (180*i)/seg);
	draw_vertex_color(e.tail.x+pt[0], e.tail.y+pt[1], col, alpha);
}

// Draw right edge
//draw_vertex_color(e.head.x-lv[0]+err[0], e.head.y-lv[1]+err[1], col, alpha);
var ptt = scr_rotate_vector(lv, (180*(seg+4))/seg);
draw_vertex_color(e.tail.x+ptt[0], e.tail.y+ptt[1], col, alpha);

// Draw semicircular outline of head
for (var i = -4; i <= seg+4; i++)
{
	var pt = scr_rotate_vector(lv, 180 + (180*i)/seg);
	draw_vertex_color(e.head.x+pt[0], e.head.y+pt[1], col, alpha);
}

// Draw left edge
//draw_vertex_color(e.tail.x+lv[0]+err[0], e.tail.y+lv[1]+err[1], col, alpha);
var ptt = scr_rotate_vector(lv, (180*(-4))/seg);
draw_vertex_color(e.tail.x+ptt[0], e.tail.y+ptt[1], col, alpha);
