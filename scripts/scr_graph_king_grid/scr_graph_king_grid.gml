/// @func scr_graph_king_grid(m, n)
/// @desc Generates the m x n king grid graph (diagraming a king's moves in chess).
...
for (var i = 0; i < m; i++)
{
	for (var j = 0; j < n; j++)
	{
		...
		// Diagonal edge, NW-to-SE
		if (j < n-1 && i < m-1)
		{
			adj[num] = [scr_grid_index(m, n, i, j), scr_grid_index(m, n, i+1, j+1)];
			num++;
		}
		
		// Diagonal edge, NE-to-SW
		if (j > 0 && i < m-1)
		{
			adj[num] = [scr_grid_index(m, n, i, j), scr_grid_index(m, n, i+1, j-1)];
			num++;
